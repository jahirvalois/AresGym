...
async function main() {
  ...
  try {
    ...
    // Create descending index on date to support ORDER BY date desc for /api/logs
    const result = await db.collection('logs').createIndex({ date: -1 });
    console.log('Created index on logs.date:', result);
  } catch (err) {
